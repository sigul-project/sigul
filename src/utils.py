...
def unbind_passphrase(bound_passphrase):
    ...
    """
    while bound_passphrase.startswith('{'):
        # If the passphrase starts with {, that means it's a bound one,
        ...
        bound_passphrase = unbind_func(value, **info)
        if bound_passphrase is None:
            # This unbinding failed
            logging.warning('Failed to unbind with method %s, args %s'
                            % (method, ','.join([str('%s=%s' %
                                                 (key, info[key]))
                                                 for key in info])))
            return None
...
def bind_passphrase(passphrase, bind_params):
    ...
    If the key is to be bound, the returned string is a json object string
    ...
    for entry in bind_params:
        ...
        value, args = bind_func(passphrase, **entry)
        if value is None:
            # This binding failed
            return None
        if args is None:
